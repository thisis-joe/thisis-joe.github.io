...
direction TB
WEB["[WebServer]<br/>정적 응답·Reverse Proxy<br/>예: Nginx, Apache"]
...
CONTROLLER["[Controller]<br/>요청 DTO 수신·응답 방향 결정<br/>예: @GetMapping"]
VALID{"[Validation]<br/>입력값 형식 검증<br/>예: email, size"}
end

subgraph BUSINESS_DATA["Business / Data Access"]
direction TB
SERVICE["[Service]<br/>비즈니스 규칙·트랜잭션 수행<br/>예: order service"]
REPO["[Repository]<br/>데이터 접근 캡슐화<br/>예: JpaRepository"]
DATA[("[Data]<br/>서비스가 사용하는 데이터 원천<br/>예: DB, Redis")]
end
...
HTML["[HTML]<br/>브라우저 문서 응답<br/>예: Thymeleaf view"]
JSON["[JSON]<br/>API 구조화 응답<br/>예: application/json"]
ERROR["[Error]<br/>마스킹된 실패 응답<br/>예: 400, 500"]
RESP["[Response]<br/>최종 HTTP 응답<br/>예: status, header"]
end
