...
SERVICE["[Service]<br/>검증된 요청으로 업무 수행<br/>예: order service"]
end

subgraph DB_BOUNDARY["Repository / DB Boundary"]
direction TB
REPO["[Repository]<br/>DB 접근·SQL 방어 지점<br/>예: prepared query"]
...
ERROR["[RejectOrMask]<br/>실패·내부 정보 마스킹<br/>예: 400, 403"]
RESPONSE["[Response]<br/>보안 검증 후 최종 응답<br/>예: safe body"]
end
