...
direction TB
USER(["[User]<br/>클러스터 밖 요청 주체<br/>예: browser user"])
INGRESS[["[Ingress]<br/>HTTP 라우팅 진입점<br/>예: host/path rule"]]
SERVICE[["[Service]<br/>Pod 집합의 안정적 접근점<br/>예: ClusterIP"]]
end

subgraph WORKLOAD["Workload"]
direction TB
DEPLOY[["[Deployment]<br/>Pod 수와 업데이트 관리<br/>예: rolling update"]]
REPLICASET[["[ReplicaSet]<br/>Pod 복제 수 유지<br/>예: replicas=3"]]
POD["[Pod]<br/>컨테이너 실행 단위<br/>예: app pod"]
CONTAINER["[Container]<br/>애플리케이션 프로세스<br/>예: spring boot"]
end

subgraph CONFIG["Configuration"]
direction TB
CONFIGMAP[("[ConfigMap]<br/>일반 설정<br/>예: profile, flag")]
SECRET[("[Secret]<br/>민감 설정<br/>예: password, token")]
PROBE{"[Probe]<br/>상태 확인 규칙<br/>예: liveness, readiness"}
end
...
- [ConfigMap Secret Volume 상세](<../Web-Operations/14. Kubernetes 입문과 도입 판단/02. ConfigMap Secret Volume 상세.md>)
