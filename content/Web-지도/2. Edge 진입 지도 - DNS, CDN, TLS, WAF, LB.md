...
CACHEDRESP["[CachedResponse]<br/>CDN에서 브라우저로<br/>정적 응답 종료"]
end
...
BLOCK["[Blocked]<br/>보안 정책에 의해<br/>요청이 차단된 상태"]
LB[["[LB]<br/>정상 서버로 요청 전달<br/>예: health, route"]]
end

subgraph ORIGIN["Origin Target"]
direction TB
WEB["[OriginTarget]<br/>WebServer 또는 WAS<br/>정적 응답 또는 애플리케이션 진입점"]
end
...
linkStyle 2,3 stroke:#9E9E9E,stroke-width:1.5px;
