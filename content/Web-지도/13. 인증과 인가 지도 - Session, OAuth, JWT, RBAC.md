...
direction TB
USER(["[User]<br/>로그인 시도"])
BROWSER["[Browser/App]<br/>인증 정보 보관·전송"]
end

subgraph AUTHN["Authentication"]
direction TB
LOGIN["[Login]<br/>ID/PW, Social Login"]
IDP[["[OAuth/OIDC Provider]<br/>외부 신원 제공자"]]
SESSION["[Session]<br/>서버 측 로그인 상태"]
JWT["[JWT]<br/>서명된 claim 토큰"]
end

subgraph AUTHZ["Authorization"]
direction TB
FILTER[["[SecurityFilter]<br/>인증 정보 검증"]]
RBAC{"[RBAC]<br/>Role 기반 권한 판단"}
RESOURCE["[ProtectedResource]<br/>API, Page, Data"]
DENY["[Deny]<br/>401 또는 403"]
end
...
linkStyle 9,10 stroke:#C62828,stroke-width:2px;
```
---
