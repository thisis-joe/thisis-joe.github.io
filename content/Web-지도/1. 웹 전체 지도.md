...
USER(["[User]<br/>서비스를 사용하는 주체<br/>예: click, submit"])
...
BACKUP[("[Backup]<br/>복구용 사본<br/>예: snapshot, dump")]
end
...
direction TB
RESPONSE["[Response]<br/>최종 HTTP 응답<br/>예: HTML, JSON"]
RENDER["[Render]<br/>브라우저 화면 갱신<br/>예: DOM render"]
end

subgraph OPS_FLOW["Ops / Background"]
direction TB
CICD[["[CI/CD]<br/>빌드·테스트·배포<br/>예: build, deploy"]]
SCHED[["[Scheduler]<br/>시간 기반 실행 트리거<br/>예: cron"]]
BATCH[["[Batch]<br/>대량 처리 작업<br/>예: 정산, 통계"]]
...
처음 공부한다면 `1 → 2 → 3 → 4 → 5 → 6 → 7 → 8 → 9` 순서로 읽는 것이 좋다. 먼저 전체 요청 흐름을 잡고, Edge 진입, Spring MVC 내부, 데이터 접근, Redis, 비동기, 배포, 운영, 보안 순서로 확대해보는 흐름이다.

실무 장애 대응을 먼저 익히고 싶다면 `16 → 8 → 17 → 2/3/4/5/6` 순서가 좋다. 먼저 장애 점검 순서를 잡고, 관측과 운영 지표를 본 뒤 Edge, WAS, 데이터, Redis, 비동기 구간으로 원인을 좁히는 흐름이다.

설계 판단을 연습하려면 `17 → 21 → 11 → 12 → 14 → 15 → 22` 순서로 보면 된다. 운영 지표와 trade-off를 기준으로 DB 정합성, Queue 신뢰성, Docker, Kubernetes, Data Pipeline 선택지를 연결해보는 경로다.

보안과 API 경계를 중심으로 보려면 `9 → 13 → 10 → 19 → 23` 순서가 자연스럽다. 보안 검증 위치를 먼저 잡고, 인증·인가, HTTP 상태 관리, API 계약, Threat Model로 확장하는 흐름이다.

프론트엔드와 AI 기능까지 연결하려면 `18 → 19 → 24` 순서로 보면 된다. 브라우저의 API 호출이 계약과 에러 응답을 거쳐 AI/LLM 서버 연동으로 확장되는 지점을 확인할 수 있다.

## Web-지도 작성 규칙

Web-지도는 상세 튜토리얼이 아니라 관계와 흐름을 보여주는 문서다. 깊은 설정법이나 구현 세부사항은 `## 관련 상세 문서`로 넘기고, 본문은 위치, 책임, 흐름, 장애 지점을 이해하는 데 집중한다.

Mermaid 노드는 `[기술명] + 역할 + 예시` 3단 구조를 기본으로 쓴다. 화살표 라벨은 `request`, `lookup`, `validate`, `route`, `query`, `publish`, `consume`, `ack`, `fallback`, `observe`처럼 흐름의 동작을 드러내야 한다.

색상은 초록은 서버 처리, 주황은 DB·Redis·Storage 같은 데이터 접근, 보라는 Queue·Event·External API 같은 비동기·외부 연동, 회색은 CI/CD·APM·Logging·Monitoring·Backup 같은 운영·관측, 빨강은 실패·차단·검증 실패, 파랑은 Browser·Response·Render 같은 클라이언트·응답 흐름에 쓴다. 굵은 선은 핵심 데이터 접근, 점선은 비동기·운영·관측 흐름에 쓴다.

장애 확인 섹션은 Mermaid의 요청 흐름 또는 바깥쪽에서 안쪽으로 들어가는 순서와 맞춘다. `## 관련 상세 문서`는 각 지도당 3~7개 정도로 엄선하고, 해당 지도의 핵심 흐름과 직접 이어지는 실제 존재 문서만 연결한다.
