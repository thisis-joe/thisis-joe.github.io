...
direction TB
JOURNEY["[UserJourney]<br/>사용자가 기대하는 핵심 흐름<br/>예: login, checkout"]
...
direction TB
LATENCY[["[Latency]<br/>응답 지연 분포<br/>예: p95, p99"]]
ERROR[["[ErrorRate]<br/>실패 비율<br/>예: 5xx, timeout"]]
SATURATION[["[Saturation]<br/>자원 포화 상태<br/>예: CPU, pool, queue"]]
FRESHNESS[["[Freshness]<br/>데이터 최신성<br/>예: lag, cache age"]]
end

subgraph DECISION["Design Trade-off"]
direction TB
SYNC["[Sync]<br/>즉시 결과·강한 정합성<br/>예: payment approval"]
ASYNC["[Async]<br/>빠른 응답·나중 처리<br/>예: email, report"]
CACHE["[Cache]<br/>빠른 조회·정합성 비용<br/>예: product list"]
REPLICA["[ScaleOut]<br/>처리량 증가·운영 복잡도<br/>예: stateless app"]
end

subgraph ACTION["Action"]
direction TB
ALERT{"[Alert]<br/>대응할 신호<br/>예: SLO burn"}
TUNE["[TuneDesign]<br/>설계·설정 조정<br/>예: timeout, pool"]
DOC["[DecisionRecord]<br/>선택 이유 기록<br/>예: ADR"]
end

JOURNEY -->|define what matters| SLI
SLI -->|set objective| SLO
SLI -->|measure latency| LATENCY
SLI -->|measure errors| ERROR
SLI -->|measure saturation| SATURATION
SLI -->|measure freshness| FRESHNESS
...
linkStyle 0,1,2,3,4,5,15,16 stroke:#455A64,stroke-width:2px;
linkStyle 6,7,8,9 stroke:#C62828,stroke-width:2px;
linkStyle 10,11,12,13,14 stroke:#6A1B9A,stroke-width:2px,stroke-dasharray:5 5;
```
---
...
- [SLI SLO Error Budget 기본 상세](<../Web-Operations/10. APM Health Check Alert/04. SLI SLO Error Budget 기본 상세.md>)
