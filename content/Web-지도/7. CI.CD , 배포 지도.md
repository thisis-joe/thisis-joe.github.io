...
direction TB
SOURCE["[Source]<br/>Git에 저장된 애플리케이션 코드,<br/>설정, 배포 정의"]
end
...
STOP["[PipelineStop]<br/>테스트 실패 시<br/>배포 전 중단"]
end
...
ROLLBACK["[Rollback]<br/>배포 후 실패 시<br/>이전 정상 버전으로 복구"]
end
...
direction TB
WEB["[WebServer]<br/>정적 파일과 Reverse Proxy를<br/>담당하는 배포 대상"]
WAS["[WAS]<br/>Spring Boot 애플리케이션이<br/>실행되는 배포 대상"]
