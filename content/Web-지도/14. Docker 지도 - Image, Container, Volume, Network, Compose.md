...
direction TB
DOCKERFILE["[Dockerfile]<br/>이미지 생성 절차"]
IMAGE[("[Image]<br/>실행 파일·런타임·설정 묶음")]
REGISTRY[("[Registry]<br/>이미지 저장소")]
end

subgraph RUN["Run"]
direction TB
CONTAINER["[Container]<br/>Image를 실행한 프로세스 환경"]
ENV["[Env]<br/>환경 변수·설정 주입"]
PORT["[PortMapping]<br/>host:container 포트 연결"]
end

subgraph STATE_NET["State / Network"]
direction TB
VOLUME[("[Volume]<br/>컨테이너 밖에 유지되는 데이터")]
NETWORK[["[Network]<br/>컨테이너 간 통신 경로"]]
COMPOSE[["[Compose]<br/>여러 서비스 실행 정의"]]
end
...
- [Linux Server 운영 기본 압축 정리](<../Web-Operations/02. Linux Server 운영 기본/00. Linux Server 운영 기본 압축 정리.md>)
