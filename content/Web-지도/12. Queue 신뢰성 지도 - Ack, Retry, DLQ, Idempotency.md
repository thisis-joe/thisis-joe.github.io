...
direction TB
WAS["[WAS]<br/>작업 메시지 생성"]
CONFIRM{"[PublishConfirm]<br/>브로커 저장 확인"}
end

subgraph BROKER["Message Broker"]
direction TB
QUEUE[["[Queue]<br/>작업 메시지 보관"]]
RETRY[["[Retry]<br/>지연 후 재시도"]]
DLQ[["[DLQ]<br/>계속 실패한 메시지 격리"]]
end

subgraph CONSUMER["Consumer"]
direction TB
WORKER["[Worker]<br/>메시지 처리"]
ACK{"[Ack/Nack]<br/>성공·실패 신호"}
IDEMPOTENT[("[IdempotencyStore]<br/>처리 이력·중복 방지")]
DB[("[DB]<br/>업무 결과 저장")]
end
