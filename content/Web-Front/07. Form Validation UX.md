...
- 서버가 보낸 field name을 `as keyof Form`으로 바로 단언하면 백엔드 응답 변경을 놓치기 쉽다. field name type guard를 두고, 매핑할 수 없는 에러는 form-level error로 보여준다.
- 관련 함정은 [[02. 백엔드 개발자를 위한 프론트엔드 문법 함정과 디버깅#number input 값이 실제로는 string인 문제를 어떻게 다룰까?]], [[02. 백엔드 개발자를 위한 프론트엔드 문법 함정과 디버깅#controlled/uncontrolled input 경고는 왜 생길까?]], [[02. 백엔드 개발자를 위한 프론트엔드 문법 함정과 디버깅#truthy/falsy와 기본값 처리는 왜 위험할까?]]를 다시 본다.
