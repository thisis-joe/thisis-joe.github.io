...
`SecurityFilterChain`은 특정 요청에 적용될 Security Filter 목록이다. `/api/**`와 `/admin/**`에 서로 다른 체인을 둘 수 있으며, 첫 번째로 매칭된 체인만 실행된다는 점이 중요하다. 어떤 체인은 필터가 거의 없거나 아예 없게 구성될 수도 있지만, 정적 리소스도 보안 헤더 같은 처리가 필요하면 완전히 ignore하기보다 `permitAll`을 우선 검토한다.
