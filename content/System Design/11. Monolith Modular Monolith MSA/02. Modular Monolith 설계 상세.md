...
>- `Modular Monolith 설계`에서 먼저 확인해야 하는 요구사항은 무엇인가?
>- 이 선택은 성능, 정합성, 장애 격리, 운영 복잡도 중 무엇을 바꾸는가?
>- Java/Spring 백엔드에서 어떤 API 계약과 안전장치를 남겨야 하는가?

## 개요

`Modular Monolith 설계`는 `Monolith Modular Monolith MSA`에서 서비스 분리 여부를 데이터 소유권, 배포 독립성, 운영 비용으로 판단하는 흐름을 다루는 문서다.

프로세스는 하나로 유지하되 패키지, 의존성, 데이터 접근 경계를 분리한다. 이 문서에서는 이 설명을 구현 팁으로 끝내지 않고, 어떤 요구사항이 어떤 구조 선택을 강제하는지 추적한다.

대표 상황은 다음과 같다. order 모듈이 payment repository를 직접 호출하지 못하게 하면 나중에 서비스 분리가 쉬워진다. 이 한 문장 안에는 사용자 경험, 트래픽 모양, 상태 owner, 실패 시 응답이 함께 들어 있다.

## 먼저 고정할 것

- `Modular Monolith 설계`의 사용자 결과를 성공, 지연, 실패 상태로 나눠 적는다.
...
`Modular Monolith 설계`를 설계할 때 위 질문 중 하나라도 답하지 못하면 그림은 그럴듯해도 운영 기준이 비어 있을 가능성이 높다.

## 흐름 예시

```mermaid
flowchart LR
    A["Monolith"] --> B["Module Boundary"]
    B["Module Boundary"] --> C["Service Boundary"]
    C["Service Boundary"] --> D["Owned DB"]
    D["Owned DB"] --> E["Event"]
    E["Event"] --> F["Deploy"]
```
...
| 선택지 | 장점 | 비용 | 적합한 조건 |
| --- | --- | --- | --- |
| monolith | 트랜잭션과 배포가 단순하다 | 변경 충돌과 blast radius가 커진다 | 작은 팀과 초기 제품 |
| modular monolith | 운영은 단순하게 두고 경계를 만든다 | 강제 규칙이 없으면 다시 얽힌다 | 분리 전 단계 |
| MSA | 독립 배포와 장애 격리가 가능하다 | 분산 트랜잭션과 관측 비용이 생긴다 | 팀과 데이터 owner가 분리될 때 |
| shared DB 금지 | 소유권이 명확하다 | 데이터 조회를 API나 event로 풀어야 한다 | 서비스 경계가 실제 책임 경계일 때 |

`Modular Monolith 설계`의 선택지는 성능만 비교하면 안 된다. 정합성, 장애 전파, 배포 난이도, 관측 지표까지 함께 비교해야 실제 운영 가능한 결정이 된다.

## 개인 프로젝트 기준

- `Modular Monolith 설계`를 왜 도입했는지 한 문장으로 남긴다.
- order 모듈이 payment repository를 직접 호출하지 못하게 하면 나중에 서비스 분리가 쉬워진다.
...
- `deploy frequency, change coupling, blast radius`를 dashboard와 alert 기준으로 연결한다.
...
- `Modular Monolith 설계`는 단독 패턴이 아니라 주변 경계와 함께 봐야 한다.
...
- `Modular Monolith 설계`가 필요한 이유를 트래픽이나 사용자 경험으로 설명하지 못한다.
...
- 확인 질문: `Modular Monolith 설계`에서 가장 먼저 물어야 할 질문은 무엇인가?
	- 답변: 사용자가 성공으로 기대하는 결과와 실패했을 때 허용할 경험을 먼저 묻는다.
- 확인 질문: 이 주제가 바꾸는 설계 축은 무엇인가?
	- 답변: 서비스 분리 여부를 데이터 소유권, 배포 독립성, 운영 비용으로 판단하는 흐름과 연결되며 성능, 정합성, 장애 격리, 운영 복잡도 중 하나 이상을 바꾼다.
