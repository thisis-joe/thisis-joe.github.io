...
>- `Latency Throughput Availability`에서 먼저 확인해야 하는 요구사항은 무엇인가?
>- 이 선택은 성능, 정합성, 장애 격리, 운영 복잡도 중 무엇을 바꾸는가?
>- Java/Spring 백엔드에서 어떤 API 계약과 안전장치를 남겨야 하는가?

## 개요

`Latency Throughput Availability`는 `Functional Non-functional Requirement`에서 기능 요구사항과 비기능 요구사항을 분리해 API 계약, SLO, 제약, 가정으로 고정하는 흐름을 다루는 문서다.

응답 시간, 처리량, 가용성은 서로 다른 지표이므로 하나의 빠름으로 묶지 않는다. 이 문서에서는 이 설명을 구현 팁으로 끝내지 않고, 어떤 요구사항이 어떤 구조 선택을 강제하는지 추적한다.

대표 상황은 다음과 같다. p95 200ms가 필요하면 DB round trip, cache hit, 외부 API 호출 위치를 함께 본다. 이 한 문장 안에는 사용자 경험, 트래픽 모양, 상태 owner, 실패 시 응답이 함께 들어 있다.

## 먼저 고정할 것

- `Latency Throughput Availability`의 사용자 결과를 성공, 지연, 실패 상태로 나눠 적는다.
...
`Latency Throughput Availability`를 설계할 때 위 질문 중 하나라도 답하지 못하면 그림은 그럴듯해도 운영 기준이 비어 있을 가능성이 높다.

## 흐름 예시

```mermaid
flowchart LR
    A["Requirement"] --> B["Functional"]
    B["Functional"] --> C["Non-functional"]
    C["Non-functional"] --> D["Constraint"]
    D["Constraint"] --> E["Assumption"]
    E["Assumption"] --> F["Review"]
```
...
```java
record RequirementReview(String feature, String slo, String constraint, String assumption) {}
```
...
```text
policy = Latency Throughput Availability
...
| 선택지 | 장점 | 비용 | 적합한 조건 |
| --- | --- | --- | --- |
| 기능만 확정 | 개발 시작이 빠르다 | 성능과 장애 기준이 나중에 흔들린다 | 프로토타입 |
| SLO 명시 | 리뷰 기준이 생긴다 | 측정과 알림이 필요하다 | 운영 서비스 |
| 가정 분리 | 불확실성을 관리한다 | 검증 일정이 필요하다 | 초기 정보가 부족할 때 |
| 제약 고정 | 설계 범위가 선명해진다 | 선택지가 줄어든다 | 보안, 비용, 법규 조건 |

`Latency Throughput Availability`의 선택지는 성능만 비교하면 안 된다. 정합성, 장애 전파, 배포 난이도, 관측 지표까지 함께 비교해야 실제 운영 가능한 결정이 된다.

## 개인 프로젝트 기준

- `Latency Throughput Availability`를 왜 도입했는지 한 문장으로 남긴다.
- p95 200ms가 필요하면 DB round trip, cache hit, 외부 API 호출 위치를 함께 본다.
...
- `latency, throughput, availability, retention`를 dashboard와 alert 기준으로 연결한다.
...
- `Latency Throughput Availability`는 단독 패턴이 아니라 주변 경계와 함께 봐야 한다.
...
- `Latency Throughput Availability`가 필요한 이유를 트래픽이나 사용자 경험으로 설명하지 못한다.
...
- 확인 질문: `Latency Throughput Availability`에서 가장 먼저 물어야 할 질문은 무엇인가?
	- 답변: 사용자가 성공으로 기대하는 결과와 실패했을 때 허용할 경험을 먼저 묻는다.
- 확인 질문: 이 주제가 바꾸는 설계 축은 무엇인가?
	- 답변: 기능 요구사항과 비기능 요구사항을 분리해 API 계약, SLO, 제약, 가정으로 고정하는 흐름과 연결되며 성능, 정합성, 장애 격리, 운영 복잡도 중 하나 이상을 바꾼다.
