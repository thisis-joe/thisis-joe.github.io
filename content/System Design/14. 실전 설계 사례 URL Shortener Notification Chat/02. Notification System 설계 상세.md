...
>- `Notification System 설계`에서 먼저 확인해야 하는 요구사항은 무엇인가?
>- 이 선택은 성능, 정합성, 장애 격리, 운영 복잡도 중 무엇을 바꾸는가?
>- Java/Spring 백엔드에서 어떤 API 계약과 안전장치를 남겨야 하는가?

## 개요

`Notification System 설계`는 `실전 설계 사례 URL Shortener Notification Chat`에서 대표 서비스를 요구사항, 트래픽, 저장소, 캐시, queue, 일관성으로 풀어내는 흐름을 다루는 문서다.

수신자 선택, 채널별 발송, retry, unsubscribe, delivery tracking을 나눈다. 이 문서에서는 이 설명을 구현 팁으로 끝내지 않고, 어떤 요구사항이 어떤 구조 선택을 강제하는지 추적한다.

대표 상황은 다음과 같다. 푸시 실패는 재시도하지만 수신 거부 사용자는 producer 단계에서 제외한다. 이 한 문장 안에는 사용자 경험, 트래픽 모양, 상태 owner, 실패 시 응답이 함께 들어 있다.

## 먼저 고정할 것

- `Notification System 설계`의 사용자 결과를 성공, 지연, 실패 상태로 나눠 적는다.
...
`Notification System 설계`를 설계할 때 위 질문 중 하나라도 답하지 못하면 그림은 그럴듯해도 운영 기준이 비어 있을 가능성이 높다.
...
```text
redirect peak = 20,000 QPS
cache hit = 95%
chat fanout room = 1,000명
notification retry = 3회
feed materialization = follower 규모별 선택
```
...
```java
record ShortenRequest(String originalUrl, String customAlias) {}
record ChatMessage(String roomId, String messageId, String body) {}
```
...
| 선택지 | 장점 | 비용 | 적합한 조건 |
| --- | --- | --- | --- |
| 핵심 경로 분리 | latency 목표가 명확해진다 | 부가 경로 추적이 필요하다 | redirect, message send 같은 핵심 API |
| cache | read peak를 흡수한다 | stale와 hot key가 생긴다 | URL redirect, profile, feed |
| queue | fanout과 외부 발송을 분리한다 | 중복과 DLQ 운영이 필요하다 | 알림, 로그, feed fanout |
| rate limit | 남용과 비용을 막는다 | 정상 사용자를 제한할 수 있다 | URL 생성, 메시지 전송, 알림 발송 |

`Notification System 설계`의 선택지는 성능만 비교하면 안 된다. 정합성, 장애 전파, 배포 난이도, 관측 지표까지 함께 비교해야 실제 운영 가능한 결정이 된다.

## 개인 프로젝트 기준

- `Notification System 설계`를 왜 도입했는지 한 문장으로 남긴다.
- 푸시 실패는 재시도하지만 수신 거부 사용자는 producer 단계에서 제외한다.
...
- `redirect latency, delivery success, fanout latency`를 dashboard와 alert 기준으로 연결한다.
...
- `Notification System 설계`는 단독 패턴이 아니라 주변 경계와 함께 봐야 한다.
...
- `Notification System 설계`가 필요한 이유를 트래픽이나 사용자 경험으로 설명하지 못한다.
...
- 확인 질문: `Notification System 설계`에서 가장 먼저 물어야 할 질문은 무엇인가?
	- 답변: 사용자가 성공으로 기대하는 결과와 실패했을 때 허용할 경험을 먼저 묻는다.
- 확인 질문: 이 주제가 바꾸는 설계 축은 무엇인가?
	- 답변: 대표 서비스를 요구사항, 트래픽, 저장소, 캐시, queue, 일관성으로 풀어내는 흐름과 연결되며 성능, 정합성, 장애 격리, 운영 복잡도 중 하나 이상을 바꾼다.
