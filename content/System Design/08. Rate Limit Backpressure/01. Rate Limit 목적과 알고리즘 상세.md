...
>- `Rate Limit 목적과 알고리즘`에서 먼저 확인해야 하는 요구사항은 무엇인가?
>- 이 선택은 성능, 정합성, 장애 격리, 운영 복잡도 중 무엇을 바꾸는가?
>- Java/Spring 백엔드에서 어떤 API 계약과 안전장치를 남겨야 하는가?

## 개요

`Rate Limit 목적과 알고리즘`는 `Rate Limit Backpressure`에서 받을 수 있는 요청량과 처리 가능한 작업량의 차이를 제한, 대기, 거절 정책으로 다루는 흐름을 다루는 문서다.

rate limit은 남용 방지뿐 아니라 핵심 기능 보호와 비용 제한을 목표로 한다. 이 문서에서는 이 설명을 구현 팁으로 끝내지 않고, 어떤 요구사항이 어떤 구조 선택을 강제하는지 추적한다.

대표 상황은 다음과 같다. 로그인 API는 IP 기준, 결제 API는 user 기준, 검색 API는 API key 기준이 맞을 수 있다. 이 한 문장 안에는 사용자 경험, 트래픽 모양, 상태 owner, 실패 시 응답이 함께 들어 있다.

## 먼저 고정할 것

- `Rate Limit 목적과 알고리즘`의 사용자 결과를 성공, 지연, 실패 상태로 나눠 적는다.
...
`Rate Limit 목적과 알고리즘`를 설계할 때 위 질문 중 하나라도 답하지 못하면 그림은 그럴듯해도 운영 기준이 비어 있을 가능성이 높다.
...
`Rate Limit 목적과 알고리즘`의 선택지는 성능만 비교하면 안 된다. 정합성, 장애 전파, 배포 난이도, 관측 지표까지 함께 비교해야 실제 운영 가능한 결정이 된다.

## 개인 프로젝트 기준

- `Rate Limit 목적과 알고리즘`를 왜 도입했는지 한 문장으로 남긴다.
- 로그인 API는 IP 기준, 결제 API는 user 기준, 검색 API는 API key 기준이 맞을 수 있다.
...
- `Rate Limit 목적과 알고리즘`는 단독 패턴이 아니라 주변 경계와 함께 봐야 한다.
...
- `Rate Limit 목적과 알고리즘`가 필요한 이유를 트래픽이나 사용자 경험으로 설명하지 못한다.
...
- 확인 질문: `Rate Limit 목적과 알고리즘`에서 가장 먼저 물어야 할 질문은 무엇인가?
