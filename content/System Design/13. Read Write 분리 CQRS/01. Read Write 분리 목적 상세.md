...
>- `Read Write 분리 목적`에서 먼저 확인해야 하는 요구사항은 무엇인가?
>- 이 선택은 성능, 정합성, 장애 격리, 운영 복잡도 중 무엇을 바꾸는가?
>- Java/Spring 백엔드에서 어떤 API 계약과 안전장치를 남겨야 하는가?

## 개요

`Read Write 분리 목적`는 `Read Write 분리 CQRS`에서 쓰기 모델과 읽기 모델을 분리할 때 projection 지연과 운영 비용을 판단하는 흐름을 다루는 문서다.

쓰기 보호와 읽기 최적화를 위해 모델과 저장소를 분리할지 판단한다. 이 문서에서는 이 설명을 구현 팁으로 끝내지 않고, 어떤 요구사항이 어떤 구조 선택을 강제하는지 추적한다.

대표 상황은 다음과 같다. 관리자 통계 조회가 주문 쓰기 DB를 잠식하면 read model을 따로 둘 수 있다. 이 한 문장 안에는 사용자 경험, 트래픽 모양, 상태 owner, 실패 시 응답이 함께 들어 있다.

## 먼저 고정할 것

- `Read Write 분리 목적`의 사용자 결과를 성공, 지연, 실패 상태로 나눠 적는다.
...
`Read Write 분리 목적`를 설계할 때 위 질문 중 하나라도 답하지 못하면 그림은 그럴듯해도 운영 기준이 비어 있을 가능성이 높다.
...
`Read Write 분리 목적`의 선택지는 성능만 비교하면 안 된다. 정합성, 장애 전파, 배포 난이도, 관측 지표까지 함께 비교해야 실제 운영 가능한 결정이 된다.

## 개인 프로젝트 기준

- `Read Write 분리 목적`를 왜 도입했는지 한 문장으로 남긴다.
- 관리자 통계 조회가 주문 쓰기 DB를 잠식하면 read model을 따로 둘 수 있다.
...
- `Read Write 분리 목적`는 단독 패턴이 아니라 주변 경계와 함께 봐야 한다.
...
- `Read Write 분리 목적`가 필요한 이유를 트래픽이나 사용자 경험으로 설명하지 못한다.
...
- 확인 질문: `Read Write 분리 목적`에서 가장 먼저 물어야 할 질문은 무엇인가?
