...
>- `Projection Read Model`에서 먼저 확인해야 하는 요구사항은 무엇인가?
>- 이 선택은 성능, 정합성, 장애 격리, 운영 복잡도 중 무엇을 바꾸는가?
>- Java/Spring 백엔드에서 어떤 API 계약과 안전장치를 남겨야 하는가?

## 개요

`Projection Read Model`는 `Read Write 분리 CQRS`에서 쓰기 모델과 읽기 모델을 분리할 때 projection 지연과 운영 비용을 판단하는 흐름을 다루는 문서다.

이벤트를 읽어 조회 전용 테이블이나 문서를 만들고 재생성 가능하게 설계한다. 이 문서에서는 이 설명을 구현 팁으로 끝내지 않고, 어떤 요구사항이 어떤 구조 선택을 강제하는지 추적한다.

대표 상황은 다음과 같다. order_summary_read_model은 주문, 결제, 배송 이벤트를 합쳐 목록 조회에 맞춘다. 이 한 문장 안에는 사용자 경험, 트래픽 모양, 상태 owner, 실패 시 응답이 함께 들어 있다.

## 먼저 고정할 것

- `Projection Read Model`의 사용자 결과를 성공, 지연, 실패 상태로 나눠 적는다.
...
`Projection Read Model`를 설계할 때 위 질문 중 하나라도 답하지 못하면 그림은 그럴듯해도 운영 기준이 비어 있을 가능성이 높다.
...
flowchart LR
    ...
    B["Write DB"] --> C["Event"]
    C["Event"] --> D["Projection Worker"]
    D["Projection Worker"] --> E["Read Model"]
    E["Read Model"] --> F["Query API"]
```
...
주문 write = 500 TPS
관리자 조회 = 3,000 QPS
projection lag 목표 = 5s
read model rebuild = 2h
lag 초과 시 stale badge
```
...
```java
@EventListener
void on(OrderPaid event) { projection.upsert(OrderSummary.from(event)); }
```
...
```json
{
  ...
  "orderId": "ord-1001",
  "version": 7,
  "occurredAt": "2026-06-30T10:15:00+09:00"
}
```

Projection은 이벤트 순서와 버전을 기준으로 갱신한다. read model은 깨질 수 있으므로 rebuild 경로가 있어야 한다.

## 선택 기준과 trade-off

| 선택지 | 장점 | 비용 | 적합한 조건 |
| --- | --- | --- | --- |
| 단일 모델 | 구현과 정합성이 쉽다 | 복잡한 조회가 write DB를 압박한다 | 단순 CRUD |
| read replica | 읽기 부하를 줄인다 | replica lag가 생긴다 | 읽기 많고 지연 허용 가능 |
| projection | 조회 모델을 최적화한다 | event replay와 rebuild가 필요하다 | 목록, 통계, feed |
| CQRS | write와 read 요구를 분리한다 | 운영 복잡도가 커진다 | 도메인 규모와 조회 복잡도가 충분할 때 |

`Projection Read Model`의 선택지는 성능만 비교하면 안 된다. 정합성, 장애 전파, 배포 난이도, 관측 지표까지 함께 비교해야 실제 운영 가능한 결정이 된다.

## 개인 프로젝트 기준

- `Projection Read Model`를 왜 도입했는지 한 문장으로 남긴다.
- order_summary_read_model은 주문, 결제, 배송 이벤트를 합쳐 목록 조회에 맞춘다.
...
- `projection lag, read latency, rebuild time`를 dashboard와 alert 기준으로 연결한다.
...
- `Projection Read Model`는 단독 패턴이 아니라 주변 경계와 함께 봐야 한다.
...
- `Projection Read Model`가 필요한 이유를 트래픽이나 사용자 경험으로 설명하지 못한다.
...
- 확인 질문: `Projection Read Model`에서 가장 먼저 물어야 할 질문은 무엇인가?
	- 답변: 사용자가 성공으로 기대하는 결과와 실패했을 때 허용할 경험을 먼저 묻는다.
- 확인 질문: 이 주제가 바꾸는 설계 축은 무엇인가?
	- 답변: 쓰기 모델과 읽기 모델을 분리할 때 projection 지연과 운영 비용을 판단하는 흐름과 연결되며 성능, 정합성, 장애 격리, 운영 복잡도 중 하나 이상을 바꾼다.
