...
>- `Read Write 비율과 Peak Traffic`에서 먼저 확인해야 하는 요구사항은 무엇인가?
>- 이 선택은 성능, 정합성, 장애 격리, 운영 복잡도 중 무엇을 바꾸는가?
>- Java/Spring 백엔드에서 어떤 API 계약과 안전장치를 남겨야 하는가?

## 개요

`Read Write 비율과 Peak Traffic`는 `Traffic Estimation Capacity Planning`에서 사용자 수와 행위 빈도를 QPS, storage, bandwidth, memory, headroom으로 바꾸는 흐름을 다루는 문서다.

읽기와 쓰기는 병목 위치가 다르므로 비율과 피크 시간대를 따로 본다. 이 문서에서는 이 설명을 구현 팁으로 끝내지 않고, 어떤 요구사항이 어떤 구조 선택을 강제하는지 추적한다.

대표 상황은 다음과 같다. 읽기 95%, 쓰기 5%라도 쓰기가 hot row를 때리면 쓰기 병목이 먼저 온다. 이 한 문장 안에는 사용자 경험, 트래픽 모양, 상태 owner, 실패 시 응답이 함께 들어 있다.

## 먼저 고정할 것

- `Read Write 비율과 Peak Traffic`의 사용자 결과를 성공, 지연, 실패 상태로 나눠 적는다.
...
`Read Write 비율과 Peak Traffic`를 설계할 때 위 질문 중 하나라도 답하지 못하면 그림은 그럴듯해도 운영 기준이 비어 있을 가능성이 높다.

## 흐름 예시

```mermaid
flowchart LR
    A["DAU"] --> B["Action Count"]
    B["Action Count"] --> C["Average QPS"]
    C["Average QPS"] --> D["Peak QPS"]
    D["Peak QPS"] --> E["Capacity"]
    E["Capacity"] --> F["Headroom"]
```
...
DAU = 1,000,000
사용자당 일 조회 = 20
평균 QPS = 1,000,000 * 20 / 86,400 = 약 231
peak factor = 8
계획 QPS = 231 * 8 * 2 = 약 3,696
```
...
```java
long peakQps = averageQps * peakFactor;
long plannedQps = peakQps * headroomFactor;
```
...
```text
policy = Read Write 비율과 Peak Traffic
...
| 선택지 | 장점 | 비용 | 적합한 조건 |
| --- | --- | --- | --- |
| 평균 계산 | 빠르게 감을 잡는다 | 피크와 재시도 부하를 놓친다 | 초기 추정 |
| peak factor | 용량 기준이 보수적이다 | 과잉 비용 가능성이 있다 | 이벤트성 트래픽 |
| headroom | 장애 중 여유를 만든다 | 상시 비용이 늘어난다 | SLO가 있는 서비스 |
| 측정 기반 보정 | 현실에 맞게 조정한다 | 초기 데이터가 필요하다 | 운영 중 서비스 |

`Read Write 비율과 Peak Traffic`의 선택지는 성능만 비교하면 안 된다. 정합성, 장애 전파, 배포 난이도, 관측 지표까지 함께 비교해야 실제 운영 가능한 결정이 된다.

## 개인 프로젝트 기준

- `Read Write 비율과 Peak Traffic`를 왜 도입했는지 한 문장으로 남긴다.
- 읽기 95%, 쓰기 5%라도 쓰기가 hot row를 때리면 쓰기 병목이 먼저 온다.
...
- `average QPS, peak QPS, GB/day, Mbps`를 dashboard와 alert 기준으로 연결한다.
...
- `Read Write 비율과 Peak Traffic`는 단독 패턴이 아니라 주변 경계와 함께 봐야 한다.
...
- `Read Write 비율과 Peak Traffic`가 필요한 이유를 트래픽이나 사용자 경험으로 설명하지 못한다.
...
- 확인 질문: `Read Write 비율과 Peak Traffic`에서 가장 먼저 물어야 할 질문은 무엇인가?
	- 답변: 사용자가 성공으로 기대하는 결과와 실패했을 때 허용할 경험을 먼저 묻는다.
- 확인 질문: 이 주제가 바꾸는 설계 축은 무엇인가?
	- 답변: 사용자 수와 행위 빈도를 QPS, storage, bandwidth, memory, headroom으로 바꾸는 흐름과 연결되며 성능, 정합성, 장애 격리, 운영 복잡도 중 하나 이상을 바꾼다.
