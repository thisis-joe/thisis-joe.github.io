...
>- `Partitioning과 Sharding 기본`에서 먼저 확인해야 하는 요구사항은 무엇인가?
>- 이 선택은 성능, 정합성, 장애 격리, 운영 복잡도 중 무엇을 바꾸는가?
>- Java/Spring 백엔드에서 어떤 API 계약과 안전장치를 남겨야 하는가?

## 개요

`Partitioning과 Sharding 기본`는 `Scale Up Scale Out Partitioning Sharding`에서 리소스 한계를 수직 확장, 수평 확장, 파티셔닝, 샤딩 중 무엇으로 풀지 정하는 흐름을 다루는 문서다.

데이터를 나누는 기준은 조회 패턴, write 분산, 재배치 비용을 함께 본다. 이 문서에서는 이 설명을 구현 팁으로 끝내지 않고, 어떤 요구사항이 어떤 구조 선택을 강제하는지 추적한다.

대표 상황은 다음과 같다. member_id shard는 회원별 조회에는 좋지만 글로벌 최신 주문 조회에는 불리하다. 이 한 문장 안에는 사용자 경험, 트래픽 모양, 상태 owner, 실패 시 응답이 함께 들어 있다.

## 먼저 고정할 것

- `Partitioning과 Sharding 기본`의 사용자 결과를 성공, 지연, 실패 상태로 나눠 적는다.
...
`Partitioning과 Sharding 기본`를 설계할 때 위 질문 중 하나라도 답하지 못하면 그림은 그럴듯해도 운영 기준이 비어 있을 가능성이 높다.

## 흐름 예시

```mermaid
flowchart LR
    A["Load Balancer"] --> B["App Pool"]
    B["App Pool"] --> C["Partition Router"]
    C["Partition Router"] --> D["Shard A"]
    D["Shard A"] --> E["Shard B"]
    E["Shard B"] --> F["Rebalancing"]
```
...
```java
ShardKey key = ShardKey.of(memberId);
DataSource ds = shardRouter.route(key);
```
...
```text
policy = Partitioning과 Sharding 기본
...
| 선택지 | 장점 | 비용 | 적합한 조건 |
| --- | --- | --- | --- |
| scale up | 운영 변화가 작다 | 상한과 비용이 빠르게 커진다 | 단일 노드 병목 초기 |
| app scale out | stateless API 처리량을 늘린다 | DB와 외부 의존성 압박이 커진다 | 앱 CPU 병목 |
| partitioning | 데이터 접근 범위를 줄인다 | key 선택이 성능을 좌우한다 | 대용량 테이블 |
| sharding | write 분산이 가능하다 | routing, rebalancing, cross-shard query가 어렵다 | 단일 DB write 한계 |

`Partitioning과 Sharding 기본`의 선택지는 성능만 비교하면 안 된다. 정합성, 장애 전파, 배포 난이도, 관측 지표까지 함께 비교해야 실제 운영 가능한 결정이 된다.

## 개인 프로젝트 기준

- `Partitioning과 Sharding 기본`를 왜 도입했는지 한 문장으로 남긴다.
- member_id shard는 회원별 조회에는 좋지만 글로벌 최신 주문 조회에는 불리하다.
...
- `CPU, memory, shard skew, per-partition QPS`를 dashboard와 alert 기준으로 연결한다.
...
- `Partitioning과 Sharding 기본`는 단독 패턴이 아니라 주변 경계와 함께 봐야 한다.
...
- `Partitioning과 Sharding 기본`가 필요한 이유를 트래픽이나 사용자 경험으로 설명하지 못한다.
...
- 확인 질문: `Partitioning과 Sharding 기본`에서 가장 먼저 물어야 할 질문은 무엇인가?
	- 답변: 사용자가 성공으로 기대하는 결과와 실패했을 때 허용할 경험을 먼저 묻는다.
- 확인 질문: 이 주제가 바꾸는 설계 축은 무엇인가?
	- 답변: 리소스 한계를 수직 확장, 수평 확장, 파티셔닝, 샤딩 중 무엇으로 풀지 정하는 흐름과 연결되며 성능, 정합성, 장애 격리, 운영 복잡도 중 하나 이상을 바꾼다.
