...
>- `Session Store와 Sticky Session 판단`에서 먼저 확인해야 하는 요구사항은 무엇인가?
>- 이 선택은 성능, 정합성, 장애 격리, 운영 복잡도 중 무엇을 바꾸는가?
>- Java/Spring 백엔드에서 어떤 API 계약과 안전장치를 남겨야 하는가?

## 개요

`Session Store와 Sticky Session 판단`는 `Stateless Stateful Data Ownership`에서 상태 저장 위치와 데이터 소유권을 통해 수평 확장과 복구 비용을 판단하는 흐름을 다루는 문서다.

sticky session은 쉬운 완화지만 장애와 배포에 취약해 session store와 비교해야 한다. 이 문서에서는 이 설명을 구현 팁으로 끝내지 않고, 어떤 요구사항이 어떤 구조 선택을 강제하는지 추적한다.

대표 상황은 다음과 같다. 로드밸런서 sticky가 풀리면 메모리 세션 사용자는 인증 상태를 잃는다. 이 한 문장 안에는 사용자 경험, 트래픽 모양, 상태 owner, 실패 시 응답이 함께 들어 있다.

## 먼저 고정할 것

- `Session Store와 Sticky Session 판단`의 사용자 결과를 성공, 지연, 실패 상태로 나눠 적는다.
...
`Session Store와 Sticky Session 판단`를 설계할 때 위 질문 중 하나라도 답하지 못하면 그림은 그럴듯해도 운영 기준이 비어 있을 가능성이 높다.
...
flowchart LR
    ...
    B["Load Balancer"] --> C["Stateless Spring App"]
    C["Stateless Spring App"] --> D["Session Store"]
    D["Session Store"] --> E["Owner DB"]
    E["Owner DB"] --> F["Recovery"]
```
...
로그인 사용자 = 200,000
세션 평균 크기 = 2KB
필요 session memory = 약 400MB + overhead
```
...
```text
policy = Session Store와 Sticky Session 판단
...
| 선택지 | 장점 | 비용 | 적합한 조건 |
| --- | --- | --- | --- |
| 메모리 상태 | 구현이 쉽다 | scale out과 failover가 어렵다 | 단일 인스턴스 실험 |
| session store | 인스턴스 확장이 쉬워진다 | Redis 장애와 TTL을 관리해야 한다 | 로그인 서비스 |
| owner service | 책임이 명확하다 | 조회 통합이 어려워진다 | 도메인 경계가 클 때 |
| event 동기화 | 느슨한 결합이 가능하다 | 지연과 중복 처리가 필요하다 | 서비스 분리 이후 |

`Session Store와 Sticky Session 판단`의 선택지는 성능만 비교하면 안 된다. 정합성, 장애 전파, 배포 난이도, 관측 지표까지 함께 비교해야 실제 운영 가능한 결정이 된다.

## 개인 프로젝트 기준

- `Session Store와 Sticky Session 판단`를 왜 도입했는지 한 문장으로 남긴다.
- 로드밸런서 sticky가 풀리면 메모리 세션 사용자는 인증 상태를 잃는다.
...
- `session hit ratio, failover time, ownership count`를 dashboard와 alert 기준으로 연결한다.
...
- `Session Store와 Sticky Session 판단`는 단독 패턴이 아니라 주변 경계와 함께 봐야 한다.
...
- `Session Store와 Sticky Session 판단`가 필요한 이유를 트래픽이나 사용자 경험으로 설명하지 못한다.
...
- 확인 질문: `Session Store와 Sticky Session 판단`에서 가장 먼저 물어야 할 질문은 무엇인가?
	- 답변: 사용자가 성공으로 기대하는 결과와 실패했을 때 허용할 경험을 먼저 묻는다.
- 확인 질문: 이 주제가 바꾸는 설계 축은 무엇인가?
	- 답변: 상태 저장 위치와 데이터 소유권을 통해 수평 확장과 복구 비용을 판단하는 흐름과 연결되며 성능, 정합성, 장애 격리, 운영 복잡도 중 하나 이상을 바꾼다.
