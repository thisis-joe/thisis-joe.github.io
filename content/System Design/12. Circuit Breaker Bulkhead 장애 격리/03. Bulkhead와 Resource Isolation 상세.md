...
>- `Bulkhead와 Resource Isolation`에서 먼저 확인해야 하는 요구사항은 무엇인가?
>- 이 선택은 성능, 정합성, 장애 격리, 운영 복잡도 중 무엇을 바꾸는가?
>- Java/Spring 백엔드에서 어떤 API 계약과 안전장치를 남겨야 하는가?

## 개요

`Bulkhead와 Resource Isolation`는 `Circuit Breaker Bulkhead 장애 격리`에서 부분 장애가 전체 장애로 번지지 않도록 호출, 스레드, 커넥션을 격리하는 흐름을 다루는 문서다.

리소스 풀을 분리해 한 의존성 장애가 전체 스레드나 커넥션을 잡아먹지 않게 한다. 이 문서에서는 이 설명을 구현 팁으로 끝내지 않고, 어떤 요구사항이 어떤 구조 선택을 강제하는지 추적한다.

대표 상황은 다음과 같다. 배송 조회용 thread pool과 주문 저장용 thread pool을 분리하면 핵심 경로를 보호할 수 있다. 이 한 문장 안에는 사용자 경험, 트래픽 모양, 상태 owner, 실패 시 응답이 함께 들어 있다.

## 먼저 고정할 것

- `Bulkhead와 Resource Isolation`의 사용자 결과를 성공, 지연, 실패 상태로 나눠 적는다.
...
`Bulkhead와 Resource Isolation`를 설계할 때 위 질문 중 하나라도 답하지 못하면 그림은 그럴듯해도 운영 기준이 비어 있을 가능성이 높다.
...
flowchart LR
    ...
    B["Circuit Breaker"] --> C["Bulkhead"]
    C["Bulkhead"] --> D["External API"]
    D["External API"] --> E["Fallback"]
    E["Fallback"] --> F["Alert"]
```
...
@Bulkhead(name = "shippingClient")
public ShippingStatus getStatus(String orderId) { return client.get(orderId); }
```
...
```yaml
resilience4j.circuitbreaker.instances.shippingClient:
  failure-rate-threshold: 50
  slow-call-duration-threshold: 2s
  wait-duration-in-open-state: 30s
```

threshold는 감이 아니라 지표와 사용자 경험으로 잡는다. 너무 민감하면 정상 호출도 막고, 너무 둔하면 스레드 고갈 후에 열린다.

## 선택 기준과 trade-off

| 선택지 | 장점 | 비용 | 적합한 조건 |
| --- | --- | --- | --- |
| timeout | 무한 대기를 막는다 | 실패 응답이 늘 수 있다 | 모든 외부 호출 기본 |
| circuit breaker | 실패 의존성 호출을 줄인다 | fallback 품질이 필요하다 | 반복 실패와 slow call이 있을 때 |
| bulkhead | 리소스 전파를 막는다 | pool 크기와 queue 정책을 관리해야 한다 | 핵심 API와 부가 API를 분리할 때 |
| degradation | 핵심 기능을 살린다 | 사용자에게 축소 상태를 보여줘야 한다 | 부분 장애를 허용할 때 |

`Bulkhead와 Resource Isolation`의 선택지는 성능만 비교하면 안 된다. 정합성, 장애 전파, 배포 난이도, 관측 지표까지 함께 비교해야 실제 운영 가능한 결정이 된다.

## 개인 프로젝트 기준

- `Bulkhead와 Resource Isolation`를 왜 도입했는지 한 문장으로 남긴다.
- 배송 조회용 thread pool과 주문 저장용 thread pool을 분리하면 핵심 경로를 보호할 수 있다.
...
- `open circuit count, slow call rate, fallback rate`를 dashboard와 alert 기준으로 연결한다.
...
- `Bulkhead와 Resource Isolation`는 단독 패턴이 아니라 주변 경계와 함께 봐야 한다.
...
- `Bulkhead와 Resource Isolation`가 필요한 이유를 트래픽이나 사용자 경험으로 설명하지 못한다.
...
- 확인 질문: `Bulkhead와 Resource Isolation`에서 가장 먼저 물어야 할 질문은 무엇인가?
	- 답변: 사용자가 성공으로 기대하는 결과와 실패했을 때 허용할 경험을 먼저 묻는다.
- 확인 질문: 이 주제가 바꾸는 설계 축은 무엇인가?
	- 답변: 부분 장애가 전체 장애로 번지지 않도록 호출, 스레드, 커넥션을 격리하는 흐름과 연결되며 성능, 정합성, 장애 격리, 운영 복잡도 중 하나 이상을 바꾼다.
