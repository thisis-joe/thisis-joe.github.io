...
HikariCP의 `connectionTimeout`은 SQL 실행 제한이 아니라 “풀에서 빌리기 위해 기다리는 시간”이다. 이 차이를 모르면 connection timeout을 slow query timeout처럼 오해하게 된다.
...
DB `max_connections`가 150이라면 수치상 가능해 보여도 failover, 배포 중 중복 인스턴스, connection leak, 운영 접속 여유를 감안하면 빠듯할 수 있다.
