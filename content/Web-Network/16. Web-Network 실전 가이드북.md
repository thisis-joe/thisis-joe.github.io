...
### 보충 Case 1. `Accept`와 `Content-Type`은 맞아 보이는데 415/406이 난다

#### 상황
- 파트너는 “JSON으로 보냈다”고 말하지만 API는 `415 Unsupported Media Type`을 낸다.
- 또는 서버는 정상 응답을 만들었는데 client가 `406 Not Acceptable`을 받는다.
- local curl은 성공하지만 gateway를 지나면 header가 달라진다.

#### 관찰할 증거 또는 확인할 단서
- 실제 wire에 나간 `Content-Type`, `Accept`, `Content-Encoding`, `Accept-Encoding`을 본다.
...
- 실패 요청은 client SDK가 아니라 `curl`로 다시 만들어 header 차이를 줄인다.
...
- gateway나 WAF가 `Content-Type`을 제거하거나 normalize하는지 access log와 config를 함께 확인한다.

#### 정리
- `415`는 “내가 보낸 body를 서버가 해석할 수 없음”이고, `406`은 “client가 받을 수 있다고 한 표현으로 서버가 응답할 수 없음”이다.

#### 가져갈 한 문장
- media type 장애는 JSON 여부가 아니라 요청과 응답 표현 계약이 실제 경로에서 보존됐는지의 문제다.

### 보충 Case 2. `ETag`와 `If-Match`로 동시 수정 충돌을 막아야 한다
...
#### 관찰할 증거 또는 확인할 단서
- 조회 응답에 `ETag`가 있는지 확인한다.
- 수정 요청에 `If-Match`가 들어오는지 확인한다.
- 충돌 시 `409 Conflict`와 `412 Precondition Failed` 중 어떤 의미가 맞는지 나눈다.
...
#### 실전 행동
- 조회 응답은 현재 버전을 `ETag`로 돌려주고, 수정 요청은 `If-Match`를 요구한다.
...
- 버전 불일치는 조용한 overwrite가 아니라 `412 Precondition Failed`와 재조회 안내로 표현한다.
- `ETag`가 cache 최적화용인지, write precondition용인지 API 문서에서 분리해 설명한다.
...
#### 상황
- 인증 관련 header와 cookie가 늘어나면서 일부 요청이 `431 Request Header Fields Too Large`를 낸다.
- gateway access log에 `Authorization`, `Cookie`, `Set-Cookie`, `X-Api-Key`가 그대로 남을 수 있다.
...
| [[01. 웹 요청 흐름 전체 지도/04. 로그와 Trace ID로 따라가기 상세|로그와 Trace ID로 따라가기 상세]] | 1, 16, 보충 7 | request id로 계층별 로그 연결과 header redaction 판단 |
...
| [[02. URL URI DNS/03. DNS TTL과 배포 전파 상세|DNS TTL과 배포 전파 상세]] | 2, 14, 보충 6 | DNS 전환, 변경 요청, rollback 판단 |
...
| [[03. IP TCP UDP 기본/03. UDP와 QUIC 기본 상세|UDP와 QUIC 기본 상세]] | 15, 보충 3 | HTTP/3, QUIC, UDP fallback 판단 |
| [[03. IP TCP UDP 기본/04. OS 네트워크 관찰 상세|OS 네트워크 관찰 상세]] | 10, 보충 8 | socket, FD, port, conntrack 관찰 |
...
| [[04. TCP 연결 Keep-Alive Connection Pool/02. HTTP Client Connection Pool 상세|HTTP Client Connection Pool 상세]] | 1, 5, 12, 보충 5 | external call pool pending과 timeout 판단 |
...
| [[05. HTTP 기본 구조/01. 요청과 응답 메시지 상세|요청과 응답 메시지 상세]] | 12, 16, 보충 1, 보충 7 | request/response 구조와 request id 판단 |
| [[05. HTTP 기본 구조/02. Message Framing과 Body 상세|Message Framing과 Body 상세]] | 6 | upload/body/chunked/framing 판단 |
| [[05. HTTP 기본 구조/03. HTTP 버전별 차이 상세|HTTP 버전별 차이 상세]] | 15, 보충 3 | HTTP/1.1, HTTP/2, HTTP/3 차이 판단 |
| [[05. HTTP 기본 구조/04. Content Negotiation과 Encoding 상세|Content Negotiation과 Encoding 상세]] | 18, 보충 1 | `Vary`, compression, content negotiation 판단 |
...
| [[06. HTTP Method Status Code Header/01. Method와 멱등성 상세|Method와 멱등성 상세]] | 12, 보충 2 | retry 가능성과 method 의미 판단 |
| [[06. HTTP Method Status Code Header/02. Status Code 운영 해석 상세|Status Code 운영 해석 상세]] | 6, 8, 19, 보충 2 | status code를 응답 주체와 다음 확인으로 연결 |
| [[06. HTTP Method Status Code Header/03. Header 설계와 관찰 상세|Header 설계와 관찰 상세]] | 13, 18, 보충 1, 보충 7 | header 신뢰 경계와 관측 기준 |
| [[06. HTTP Method Status Code Header/04. API 응답 계약 상세|API 응답 계약 상세]] | 12, 13, 보충 1 | client 행동을 안내하는 response contract |
| [[07. HTTPS TLS 인증서/00. HTTPS TLS 인증서 압축 정리|HTTPS TLS 인증서 압축 정리]] | 4, 15 | TLS/인증서 전체 빠른 지도 |
| [[07. HTTPS TLS 인증서/01. TLS Handshake 상세|TLS Handshake 상세]] | 4, 15, 보충 3 | handshake timing, ALPN, TLS policy 판단 |
| [[07. HTTPS TLS 인증서/02. 인증서 Chain과 SNI 상세|인증서 Chain과 SNI 상세]] | 4, 11, 보충 4 | chain, SAN, SNI, truststore 판단 |
| [[07. HTTPS TLS 인증서/03. TLS Termination과 mTLS 상세|TLS Termination과 mTLS 상세]] | 4, 17, 보충 4 | termination 위치, mTLS, forwarded proto 판단 |
| [[07. HTTPS TLS 인증서/04. TLS 디버깅 상세|TLS 디버깅 상세]] | 4, 16, 보충 8 | TLS 장애 재현 명령과 report evidence |
...
| [[09. Cache CDN Conditional Request/02. ETag와 Conditional Request 상세|ETag와 Conditional Request 상세]] | 2, 19, 보충 2 | ETag, 304, conditional request 판단 |
| [[09. Cache CDN Conditional Request/03. CDN Origin과 Purge 상세|CDN Origin과 Purge 상세]] | 2, 14 | CDN cache key, purge, origin 비교 |
| [[09. Cache CDN Conditional Request/04. Cache 정합성 장애 상세|Cache 정합성 장애 상세]] | 2, 19, 보충 2 | stale data, purge event, 권한/가격 정합성 |
...
| [[10. Proxy Reverse Proxy Gateway/01. Proxy와 Reverse Proxy 역할 상세|Proxy와 Reverse Proxy 역할 상세]] | 8, 17, 보충 7 | 응답 주체와 reverse proxy 역할 판단 |
| [[10. Proxy Reverse Proxy Gateway/02. NGINX Timeout과 Buffer 상세|NGINX Timeout과 Buffer 상세]] | 6, 7, 15, 보충 3 | timeout, buffer, upload, long connection 판단 |
...
| [[10. Proxy Reverse Proxy Gateway/04. API Gateway와 WAF 상세|API Gateway와 WAF 상세]] | 13, 보충 6 | WAF, rate limit, gateway policy 판단 |
...
| [[11. Load Balancer Health Check Traffic Routing/01. LB 알고리즘과 Routing 상세|LB 알고리즘과 Routing 상세]] | 9, 14, 보충 6 | canary, weighted routing, target group 판단 |
...
| [[12. Timeout Retry Idempotency Circuit Breaker/01. Timeout 종류와 정렬 상세|Timeout 종류와 정렬 상세]] | 1, 5, 보충 5 | timeout chain과 정렬 판단 |
| [[12. Timeout Retry Idempotency Circuit Breaker/02. Retry와 장애 증폭 상세|Retry와 장애 증폭 상세]] | 5, 12, 13, 보충 5 | retry storm, Retry-After, budget 판단 |
...
| [[12. Timeout Retry Idempotency Circuit Breaker/05. Spring과 Resilience 설정 상세|Spring과 Resilience 설정 상세]] | 5, 보충 5 | Spring client timeout과 Resilience4j 설정 |
| [[13. 대용량 트래픽과 네트워크 병목/00. 대용량 트래픽과 네트워크 병목 압축 정리|대용량 트래픽과 네트워크 병목 압축 정리]] | 9, 10 | 대용량 병목 빠른 지도 |
| [[13. 대용량 트래픽과 네트워크 병목/01. Capacity Planning 상세|Capacity Planning 상세]] | 9, 보충 8 | SLO, headroom, shared resource 계산 |
...
| [[13. 대용량 트래픽과 네트워크 병목/05. 희귀하지만 중요한 한계 상세|희귀하지만 중요한 한계 상세]] | 10, 보충 8 | ephemeral port, NAT, DNS, FD, conntrack 한계 |
...
| [[15. 백엔드 개발자와 인프라 협업/02. 팀 서비스 운영에서 챙길 네트워크 기준 상세|팀 서비스 운영에서 챙길 네트워크 기준 상세]] | 16, 17, 보충 8 | 팀 공통 운영 계약과 runbook |
| [[15. 백엔드 개발자와 인프라 협업/03. 기업 운영 환경에서 챙길 네트워크 기준 상세|기업 운영 환경에서 챙길 네트워크 기준 상세]] | 14, 17, 보충 4, 보충 6 | 변경 관리, SLO, 보안, multi-AZ 판단 |
| [[15. 백엔드 개발자와 인프라 협업/04. 인프라 팀과 소통하는 방법 상세|인프라 팀과 소통하는 방법 상세]] | 13, 16, 20, 보충 4, 보충 6 | 인프라 요청 정보와 증거 전달 |
