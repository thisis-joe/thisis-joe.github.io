...
먼저 증상별 가능한 원인부터 짚어보자. 문서를 다 읽고 나면 
...
즉 Spring Security는 Controller 앞에서 이런 질문을 먼저 던진다.
...
## 그래서 401/403만 보는 것은 부족하다

Spring Security를 배울 때 가장 자주 보는 응답은 `401`과 `403`이다.

하지만 보안 설정 때문에 관찰되는 증상이 딱 두 개만 있는 것은 아니다.

보호 API가 예상대로 동작하지 않을 때는 여러 증상이 나올 수 있다.
...
즉, 핵심은 `401/403`이라는 숫자 자체가 아니다.

핵심은 이것이다.

```text
Controller 전에 요청이 끝날 수 있다.
```

그리고 그 이유가 인증/인가뿐 아니라 CORS, CSRF, redirect, matcher, 예외 처리일 수도 있다.

---

## 예시 1. 토큰 없는 보호 API 요청
...
이 경우 Controller 문제가 아니다.

먼저 봐야 할 것은 Security FilterChain이다.
