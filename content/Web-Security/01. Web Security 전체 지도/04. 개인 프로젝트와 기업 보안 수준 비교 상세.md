...
개인 프로젝트와 기업 보안의 차이는 “개인은 대충 해도 된다”가 아니다. 차이는 보호해야 할 자산의 규모, 영향 범위, 감사 요구, 운영 인력, 사고 대응 책임에서 나온다. 작은 서비스에서도 비밀번호 원문 저장, HTTPS 미사용, 권한 테스트 부재, 민감 로그 노출은 허용할 수 없다.
...
- 주니어는 개인 프로젝트에서도 비밀번호 해시와 권한 테스트를 생략하지 않는다.
- 주니어는 로그에 token 원문과 비밀번호가 찍히지 않는지 직접 확인한다.
- 시니어는 “지금은 작다”는 이유로 경계가 섞이지 않게 한다.
- 시니어는 나중에 승인, 감사, 탐지를 붙일 수 있도록 이벤트와 권한 모델을 분리한다.
- guru급 판단은 비용을 고려하되 사고 때 설명 가능한 최소 장치를 남기는 것이다.
