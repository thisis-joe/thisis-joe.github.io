...
- 주니어는 인증 성공을 권한 성공으로 착각하지 않아야 한다.
- 주니어는 CORS, CSRF, JWT, Session이 각각 막는 공격을 한 문장으로 설명할 수 있어야 한다.
- 시니어는 보안 설정을 저장 위치와 전송 방식 기준으로 리뷰한다.
- 시니어는 브라우저 정책과 Service 권한 정책이 모두 테스트되는지 확인한다.
- guru급 판단은 한 방어선이 깨졌을 때 다음 방어선이 피해를 줄이는지 확인하는 데 있다.
