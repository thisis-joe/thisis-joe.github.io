...
- 주니어는 “로그인했는가”와 “이 리소스에 접근 가능한가”를 분리해 구현해야 한다.
- 주니어는 요청 본문에 들어온 `userId`, `role`, `ownerId`를 권한 근거로 쓰지 않아야 한다.
- 시니어는 보안 경계가 Filter에만 몰려 있지 않은지, Service와 Repository까지 이어지는지 본다.
- 시니어는 예외 경로를 따로 본다. batch, internal API, admin API, webhook은 일반 사용자 API와 경계가 다르다.
- guru급 판단은 정상 흐름도 옆에 공격 흐름도를 같이 그리는 데서 나온다.
